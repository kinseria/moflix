...
function getAllSeries($connect, $itemsPage, $genreGet = NULL)
{
    ...
    $sentence = $connect->prepare("SELECT SQL_CALC_FOUND_ROWS series.*, GROUP_CONCAT(genres.genre_title) AS genre_title, AVG(rating) AS serie_rate FROM series JOIN series_genres ON series_genres.serie_id = series.serie_id LEFT JOIN series_reviews ON series_reviews.item = series.serie_id JOIN genres ON genres.genre_id = series_genres.genre_id WHERE series.serie_status = 1 AND series.serie_id IN (SELECT serie_id FROM series_genres WHERE genre_id = $genreGet) GROUP BY series.serie_id ORDER BY series.serie_date DESC LIMIT $limit, $itemsPage");

    }else{

    $sentence = $connect->prepare("SELECT SQL_CALC_FOUND_ROWS series.*, GROUP_CONCAT(genres.genre_title) AS genre_title, AVG(rating) AS serie_rate FROM series JOIN series_genres ON series_genres.serie_id = series.serie_id LEFT JOIN series_reviews ON series_reviews.item = series.serie_id JOIN genres ON genres.genre_id = series_genres.genre_id WHERE series.serie_status = 1 GROUP BY series.serie_id ORDER BY series.serie_date DESC LIMIT $limit, $itemsPage");
